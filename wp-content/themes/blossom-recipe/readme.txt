=== Blossom Recipe ===

Contributors: blossomthemes
Requires at least: 4.7
Requires PHP: 5.6
Tested up to: 6.2
Stable tag: 1.2.5
License: GNU General Public License v3 or later
License URI: http://www.gnu.org/licenses/gpl-3.0.html
Tags: food-and-drink, blog, one-column, two-columns, right-sidebar, left-sidebar, footer-widgets, custom-background, custom-header, custom-menu, custom-logo, featured-images, threaded-comments, full-width-template, rtl-language-support, translation-ready, theme-options, e-commerce

== Description ==
Blossom Recipe is a beautifully designed, feature-rich and fully responsive free WordPress theme to help you create yummy delicious food and recipe blogs. This theme can be a suitable choice for you to share your mouth-watering recipes with the world. You can create a fully functional and professional blog with Blossom Recipe free theme without writing a single line of code. This theme features a clean and attractive layout on the front-end and advanced content management system on the back-end. Furthermore, it is a speed optimized theme which provides your visitors with excellent user experience and keeps them engaged on your website. The Ultra SEO optimized, and Schema.org Markup ready codes help your website rank at the top in Google and other search engine results. This theme contains plenty of space for ads and is Google Adsense optimized to help you earn good revenue from your blog. Blossom Recipe free theme features extensive documentation to guide you create an attractive recipe/food blog from scratch. Also, it is backed up by a quick, responsive and friendly support team to help you solve any issues you get while creating a website with this theme. If you get any issue or have any questions related to the theme, you can contact the friendly support team. Check theme details at https://blossomthemes.com/downloads/blossom-recipe-free-wordpress-theme/, demo at https://blossomthemes.com/theme-demo/?theme=blossom-recipe, read the documentation at https://docs.blossomthemes.com/docs/blossom-recipe/, and get support at https://blossomthemes.com/support-ticket/.

== Installation ==

1. In your admin panel, go to Appearance > Themes and click the Add New button.
2. Click Upload Theme and Choose File, then select the theme's .zip file. Click Install Now.
3. Click Activate to use your new theme right away.

== Copyright ==

Blossom Recipe WordPress Theme, Copyright 2019 blossomthemes.com
Blossom Recipe is distributed under the terms of the GNU GPL

This program is free software: you can redistribute it and/or modify
it under the terms of the GNU General Public License as published by
the Free Software Foundation, either version 2 of the License, or
(at your option) any later version.

This program is distributed in the hope that it will be useful,
but WITHOUT ANY WARRANTY; without even the implied warranty of
MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
GNU General Public License for more details.

Blossom Recipe bundles the following third-party resources:

Owl-carousel, Copyright 2013-2017 David Deutsch 
License : MIT
Source  : https://owlcarousel2.github.io/OwlCarousel2

Owl Carousel v2 Accessibility Layer, Copyright 2016 Geoffrey Roberts
License : MIT
Source  : https://github.com/rtrvrtg/owlcarousel2-a11ylayer

animate.css, Copyright (c) 2017 Daniel Eden
License: MIT
Source: https://github.com/daneden/animate.css

normalize.css, Copyright 2012-2016 Nicolas Gallagher and Jonathan Neal
License: MIT
Source: https://necolas.github.io/normalize.css/

Font Awesome Free, by @fontawesome
Source: http://fontawesome.com/
License : https://fontawesome.com/license/free (Icons: CC BY 4.0, Fonts: SIL OFL 1.1, Code: MIT License)

Selectize, Copyright © 2013–2016 Brian Reavis & Contributors
License: Apache License, Version 2.0
Source: https://github.com/selectize/selectize.js

Sifter, Copyright © 2013–2015 Brian Reavis & Contributors
License: Apache License, Version 2.0
Source: https://github.com/brianreavis/sifter.js/

Microplugin, Copyright © 2013 Brian Reavis & Contributors
License: Apache License, Version 2.0
Source: https://github.com/brianreavis/microplugin.js

TGMPA, Copyright (c) 2011, Thomas Griffin
License: GPL-2.0+
Source: http://tgmpluginactivation.com/

Blossom Recipe Theme incorporates code from Kirki, Copyright (c) 2017 Aristeides Stathopoulos
License: https://github.com/aristath/kirki/blob/develop/LICENSE
Source: https://github.com/aristath/kirki

Webfonts Loader
Blossom Recipe local fonts feature is based on Webfonts Loader by WordPress Themes Team (WPTT). (https://github.com/WPTT/webfont-loader)
License: MIT
Source: https://github.com/WPTT/webfont-loader/blob/master/LICENSE

Some codes are taken from 
https://blog.josemcastaneda.com/2016/08/08/copy-paste-hurting-theme/ &
https://isabelcastillo.com/woocommerce-cart-icon-count-theme-header/

Some codes are taken from Twenty Twenty theme - https://wordpress.org/themes/twentytwenty/

Images used in screenshots.
Source:
https://www.foodiesfeed.com/free-food-photo/strawberry-mango-peach-smoothies/ ( for first and fourth image in screenshot )
https://www.foodiesfeed.com/free-food-photo/chicken-legs-with-tomatoes-peppers-and-oranges-4/
https://www.foodiesfeed.com/free-food-photo/blood-orange-upside-down-cake/
https://pxhere.com/en/photo/642869

License: CC0, https://www.foodiesfeed.com/license/
License: CC0, https://pxhere.com/en/license

All other images including icon image used in screenshot and images used in customizer settings are self created and are under the GPL.

== Changelog ==

    1.2.5 - July 12, 2023
        * Fixed: Minor refinements for child theme.
        
    1.2.4 - September 30, 2022
        * Fixed: tgmpa plugin issue corrected for php 8

    1.2.3 - September 09, 2022
        * Added: delicious recipe plugin search form issue

    1.2.2 - July 12, 2022
        * Added: feature to load google font locally

    1.2.1 - May 11, 2022
        * Fixed: Pagination design issue
        * Updated: Added FontAwesome icon for TikTok
        * Updated: Google Fonts library

    1.2.0 - February 22, 2021
       * Recommended AffiliateX plugin

    1.1.9 - July 09, 2021
        * Removed block widget for WordPress 5.8 compatibility
    
    1.1.8 - June 29, 2021
        * Fixed: instagram design issue 
        * Fixed: jQuery deprecated warnings
        * Added: link underline in post/page
        * Removed: static menu text

    1.1.7 - January 20, 2021
        * Fixed: recipe meta svg issue

    1.1.6 - December 25, 2020
        * Fixed: customizer load issue

    1.1.5 - November 26, 2020
        * Fixed: newsletter design issue
        * Fixed: slider control design issue

    1.1.4 - October 14, 2020
        * Modified: made compatible with Delicious Recipe plugin

    1.1.3 - September 22, 2020
        * Fixed: social media settings icon issue
        * Fixed: slider control value not showing issue
        * Modified: theme homepage link

    1.1.2 - August 13, 2020
        * Fixed : solved issue of jQuery live function

    1.1.1 - July 09, 2020
        * Fixed: accessibility design issues
        * Modified: demo link

    1.1.0 - March 17, 2020
        * Added: Owl Carousel v2 Accessibility Layer js
        * Fixed: instagram section design issue

    1.0.9 - January 29, 2020
        *Fixed: Submenu hide issue
        *Changed: Screenshot updated

    1.0.8 - December 19, 2019       
        *Fixed: Getting started support text

    1.0.7 - December 17, 2019       
        *Fixed: Typo Error
        *Added: Static CTA banner caption field
    
    1.0.6 - October 21, 2019       
        *Fixed: Breadcrumb Error
        *Fixed: Header search translation issue      
        *Fixed: Site Title alignment issue        
        *Fixed: Responsive menu toggle issue
        *Fixed: WooCommerce multi description display issue
        *Tweaked: Updated Google Fonts library

    1.0.5      
        Improved
        * make theme accessibility compatible
        * single meta controls modified

    1.0.4
        Fixed
        * newsletter section issue
        * issues of readme validater

        Improved
        * footer link changed
        * documentation link changed
 
        Added
        * wp body open tag
        * cta to add site link for social share

    1.0.3
    * Made footer link no-follow
    * fix fallback svg issue
    * added read more button in archive page and search page
    
    1.0.2
    * removed redirect to getting started page on activation
    * solved issue of menu not clicking when static banner is set

    1.0.1
    * updated copyright FAQ text, help panel text & plugin recommendation
    * added space between tags
    * fix comment section design issue

    1.0.0
    * Initial release